...
const fs = require("fs");
// === REMOVED: unused import to satisfy eslint
...
async function hasEligibility(buyerId, creatorId, productId) {
  ...
  const paid = await db.query(
    `SELECT 1
      ...
      WHERE o.buyer_id=$1 AND p.user_id=$2 AND (o.status='paid' OR o.status='completed')
